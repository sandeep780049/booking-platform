...
import { CheckCircle, Package, Calendar, CreditCard, ArrowRight, Home, ShoppingBag } from "lucide-react";
...
const CartSuccess = () => {
    ...
    useEffect(() => {
        const handlePaymentRedirect = async () => {
            ...
            try {
                const response = await fetch(`/api/itemBooking/payment/redirect?order_id=${orderId}&status=${status}`);
                const data = await response.json();

                if (data.success) {
                    setBookingDetails(data.data);
                    if (data.data.redirectStatus === 'completed' || data.data.redirectStatus === 'success') {
                ...
                >
                                        ...
                                        <div className="text-right">
                                            <p className="font-semibold">
                                                £{item.purchase 
                                                    ...
                                                    : (item.item.rentalPrice * item.quantity * item.rentalPeriod.days).toFixed(2)
                                                }
                                            </p>
                                        </div>
                                    </div>
                    ...
                    <div className="text-center mt-8 p-4 bg-blue-50 rounded-lg">
                        <p className="text-sm text-blue-800">
                            ...
                            <a href="mailto:support@yoursite.com" className="font-semibold underline">
