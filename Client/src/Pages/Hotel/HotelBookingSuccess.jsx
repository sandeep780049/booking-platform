...
export default function HotelBookingSuccess() {
    ...
    if (bookingId && !bookingDetails) {
      const fetchBookingDetails = async () => {
        ...
        try {
          ...
          toast.error("An error occurred while retrieving your booking information")
          navigate("/hotels")
        } finally {
          setIsLoading(false)
        }
      ...
      <div className="min-h-screen bg-white">
        ...
        <div className="pt-20 flex items-center justify-center min-h-[70vh]">
          <div className="text-center">
            ...
            <h2 className="text-xl font-semibold">Loading your booking details...</h2>
          </div>
        </div>
      </div>
    )
  ...
  const bookingDate = bookingDetails.createdAt ? new Date(bookingDetails.createdAt).toLocaleDateString() : "N/A"
  const checkInDate = hotel.startDate || bookingDetails.hotels?.[0]?.startDate || hotel.checkInDate || bookingDetails.hotels?.[0]?.checkInDate
  const checkOutDate = hotel.endDate || bookingDetails.hotels?.[0]?.endDate || hotel.checkOutDate || bookingDetails.hotels?.[0]?.checkOutDate
          ...
          <CardContent className="pt-6">
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-sm">{formattedCheckInDate}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <Calendar className="h-4 w-4 text-gray-500" />
                      <div>
                        ...
                        <p className="font-medium text-sm">{formattedCheckOutDate}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {bookingDetails.guestInfo && (
                    <div>
                      ...
                      <p className="text-sm">{bookingDetails.guestInfo.fullName}</p>
                      <p className="text-sm text-gray-600">{bookingDetails.guestInfo.email}</p>
                      <p className="text-sm text-gray-600">{bookingDetails.guestInfo.phone}</p>
                    </div>
                  )}
                </div>
              </div>
                ...
                <div className="p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono">{bookingDetails._id?.substring(0, 8) || "N/A"}</span>
                    </div>
                    <div className="flex justify-between">
            ...
            {bookingDetails.specialRequests && (
              <div className="mt-6 p-3 bg-blue-50 rounded-md">
                ...
                <p className="text-sm text-blue-700">{bookingDetails.specialRequests}</p>
              </div>
            )}
          </CardContent>
        </Card>
            ...
            <CardContent className="p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">Manage all your reservations</p>
                </div>
              </div>
